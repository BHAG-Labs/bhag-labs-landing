      ...
      <div className="gradient-border rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
          <div>
            ...
            <select value={numFounders} onChange={e => setNumFounders(Number(e.target.value))} className="w-full px-3 py-2 rounded-lg bg-secondary border border-border text-foreground text-sm">
          ...
          {Array.from({ length: numFounders }).map((_, i) => (
            <div key={i}>
              ...
              <input type="number" value={founderSplits[i]} onChange={e => { const s = [...founderSplits]; s[i] = Number(e.target.value) || 0; setFounderSplits(s); }} className="w-full px-3 py-2 rounded-lg bg-secondary border border-border text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" />
            </div>
          ))}
          <div>
          ...
          {round.enabled && (
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                <input type="number" value={round.amount} onChange={e => updateRound(i, { amount: Number(e.target.value) || 0 })} className="w-full px-3 py-2 rounded-lg bg-secondary border border-border text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" />
              </div>
              <div>
                <label className="block text-[10px] text-muted-foreground mb-1">Pre-Money Val (₹ Cr)</label>
                <input type="number" value={round.preMoney} onChange={e => updateRound(i, { preMoney: Number(e.target.value) || 0 })} className="w-full px-3 py-2 rounded-lg bg-secondary border border-border text-foreground text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" />
              </div>
              <div>
        ...
        {lastStage.postMoney && (
          <div className="mt-4 space-y-2">
            {lastStage.founders.map((f, i) => {
              ...
              return (
                <div key={i} className="flex justify-between text-xs bg-secondary rounded-lg px-3 py-2">
